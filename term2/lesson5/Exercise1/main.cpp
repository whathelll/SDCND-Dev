...
void filter(VectorXd &x, MatrixXd &P) {
	...
	for (unsigned int n = 0; n < measurements.size(); ++n) {
        ...
        MatrixXd y = z - H*x;   // (1, 1) - (1,2) * (2, 1) = (1, 1)
        MatrixXd S = H*P*H.transpose() + R;  // (1,2) * (2,2) * (2,1) + (1,1)  = (1,1)
        MatrixXd K = P*H.transpose() * S.inverse(); // (2,2)*(2,1) * (1, 1) =  (2,1)
...
//		MatrixXd K =  P * Ht * Si;
		// new state
		x = x + (K*y);   // (2, 1) + (2, 1) * (1, 1) = (2, 1)
		P = (I - K*H) * P; // ( (2, 2) - (2, 1) * (1, 2) )    * (2, 2)
		std::cout << "kf x=" << std::endl <<  x << std::endl;
        std::cout << "kf P=" << std::endl <<  P << std::endl;
...
//        std::cout << "--Ky--" << K*y << std::endl;
        ...
        x = F * x + u;
        P = F * P * F.transpose() + Q;
		std::cout << "predicted x=" << std::endl <<  x << std::endl;
		std::cout << "predicted P=" << std::endl <<  P << std::endl;
